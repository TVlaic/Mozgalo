...
from keras.layers.convolutional import SeparableConv2D, MaxPooling2D, UpSampling2D
...
def conv_block(feat_maps_out, prev):
    ...
    prev = SeparableConv2D(feat_maps_out, (3, 3), padding='same')(prev) 
    prev = BatchNormalization()(prev) # Specifying the axis and mode allows for later merging
    prev = Activation('relu')(prev)
    prev = SeparableConv2D(feat_maps_out, (3, 3), padding='same')(prev) 
    return prev
...
def skip_block(feat_maps_in, feat_maps_out, prev):
    if feat_maps_in != feat_maps_out:
        ...
        prev = SeparableConv2D(feat_maps_out, (1, 1), padding='same')(prev)
    return prev 
...
def ResidualAttention(inputs, p = 1, t = 2, r = 1):
    ...
    output_soft_mask = SeparableConv2D(num_channels, (1,1), activation='relu')(output_soft_mask)
    #final attention output
    output_soft_mask = SeparableConv2D(num_channels, (1,1), activation='sigmoid')(output_soft_mask)
...
class ResidualAttentionNetSeparableConvolutionsSmallDifferentInterpolationCenterLoss(BaseNetwork):
    def __init__(self, output_directory, checkpoint_directory, config_dict, preprocessor, name = "ResidualAttentionNetSeparableConvolutionsSmallDifferentInterpolationCenterLoss", train = True):
    ...
    def get_network(self):
        ...
        outputs = SeparableConv2D(8, (7, 7), strides = [2,2], padding='same', activation = 'relu', name = 'classification_conv_1')(outputs)
